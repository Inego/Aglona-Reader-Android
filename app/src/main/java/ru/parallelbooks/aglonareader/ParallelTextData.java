...
//	private static final float FINGERTIP = 0.1f;
	private static final float FINGERTIP = 0.2f; // 1/5 inch [crw]
...
// need for scroll calculation:
	...
	private void RenderBackground(int pairIndex, int cLine, LayoutVars lv) {
		...
		if (HighlightFirstWords && list.size() > 0 && first.x2 >= first.x1) {
	...
	private boolean NotFitOnScreen(TextPair p) {
		...
		} else if (p.renderedInfo1.line2 == -1
				  || p.renderedInfo1.line2 > LastFullScreenLine
	...
	private void RenderText(int pairIndex, int cLine, LayoutVars lv) {
		...
		int newColor =
				(NotFitOnScreen(p) || (pairIndex < CurrentPair)) ? 2 :
...
//			Log.e("PTD", "language for locale \"" + myLocale.toString() + "\" not available" );
	...
	private void DrawPositionStrip() {
		...
		canvas.drawRect((viewWidth - percentTextLength) / 2
						- borderWidth, (float) viewHeight - 12 - 2 * borderWidth,
				(viewWidth + percentTextLength) / 2 + borderWidth,
...
*/
	...
	private void ComputeNumberOfScreenLines() {
		...
		//crw LastFullScreenLine = NumberOfScreenLines - 1;
		LastFullScreenLine = NumberOfScreenLines;
...
//		mouse_text_word = found_word;
	...
	public void TurnAdvancedPopupOff() {
		...
		popUpInfo.visible = false;
		Advanced_HighlightedPair = -1;
		if (MainActivity.speakT.isSpeaking()) {
			MainActivity.speakT.stop();
		}
	}
